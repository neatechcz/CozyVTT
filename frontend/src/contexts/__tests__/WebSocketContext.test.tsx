...
describe('WebSocketProvider automatic reconnection', () => {
  ...
  it('follows a socket the client replaced by itself (server-forced reconnect) and refreshes once', async () => {
    const socket = createSocket();
    ...
    // 'io server disconnect': the old socket drops, the client builds a new one
    act(() => socket.emit('disconnect', 'io server disconnect'));
    expect(screen.getByTestId('connection-status')).toHaveTextContent('disconnected');
    socketClientMock.getSocket.mockReturnValue(replacement);
    act(() => lifecycleListeners.forEach((listener) => listener('replaced')));

    const oldEvents = socket as unknown as FakeEmitter;
    const newEvents = replacement as unknown as FakeEmitter;
    expect(oldEvents.listenerCount('authenticated')).toBe(0);
    expect(newEvents.listenerCount('authenticated')).toBe(1);
    expect((replacement.io as unknown as FakeEmitter).listenerCount('reconnect_failed')).toBe(1);

    act(() => replacement.emit('connect'));
    expect(screen.getByTestId('connection-status')).toHaveTextContent('connecting');
    act(() => replacement.emit('authenticated', { campaignId: 'c1' }));
    ...
    unmount();
    expect(newEvents.listenerCount('authenticated')).toBe(0);
    expect(lifecycleListeners.size).toBe(0);
  });

  it('ignores a socket replaced for another campaign', async () => {
    const socket = createSocket();
    ...
    socketClientMock.getSocket.mockReturnValue(foreign);
    act(() => lifecycleListeners.forEach((listener) => listener('replaced')));
